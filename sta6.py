...
def copyFolder(nrrdpath):
    ...
    if (os.path.exists(nrrdpath)):
        ...
        for dir_patient in dir_patientlist:
            ...
            # 定位到子文件夹
            if(not os.path.exists(os.path.join(patientPath, "2 Clean_Data_Window"))):
                if(os.path.exists(os.path.join(patientPath, "3 Clean_data (4 phases)"))):
                    shutil.copytree(os.path.join(patientPath, "3 Clean_data (4 phases)"), os.path.join(patientPath, "2 Clean_Data_Window"))
                    print("复制成功：", patientPath)

#文件夹重命名
def renameFolder(nrrdpath):
    # 判断路径是否存在
    if (os.path.exists(nrrdpath)):
        dir_patientlist = os.listdir(nrrdpath)
        # 遍历每一个患者
        for dir_patient in dir_patientlist:
            patientPath = os.path.join(nrrdpath, dir_patient)
            # 定位到子文件夹
            oldname = os.path.join(patientPath, "3 Clean_Data_Window")
            if(os.path.exists(oldname)):
                newname =os.path.join(patientPath, "2 Clean_Data_Window")
                os.rename(oldname, newname)

#删除文件夹
def removeFolder(nrrdpath):
    # 判断路径是否存在
    if (os.path.exists(nrrdpath)):
        dir_patientlist = os.listdir(nrrdpath)
        # 遍历每一个患者
        for dir_patient in dir_patientlist:
            patientPath = os.path.join(nrrdpath, dir_patient)
            # 定位到子文件夹
            oldname = os.path.join(patientPath, "2 Clean_data")
            if(os.path.exists(oldname)):
                #os.remove(oldname)
                #os.system('del "'+oldname+'" /F')
                os.rmdir(oldname)
...
def setContrastofNrrd(nrrdpath):
  ...
  if (os.path.exists(nrrdpath)):
    ...
    for dir_patient in dir_patientlist:
      patientPath = os.path.join(nrrdpath, dir_patient)
      #定位到子文件夹
      patientPath = os.path.join(patientPath,"2 Clean_Data_Window")
...
nrrdpath = r'N:\3 Singapore-ASTAR\2 NCCS SMART\Datasets\CT Scans\1 Full_phase (45)'
#nrrdpath = os.path.normpath(r'G:\SMART\Datasets\1 CT Scans')
#copyFolder(nrrdpath)
#setContrastofNrrd(nrrdpath)
#重命名文件夹
renameFolder(nrrdpath)
#删除文件夹
#removeFolder(nrrdpath)
